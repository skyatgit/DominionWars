...
public partial class CommandValidator : Node
{
        ...
        where T : CommandInputAttribute
    {
        ...
        data = new Dictionary<StringName, object>();
    ...
    )
    {
        ...
        foreach (CommandInputType type in argument.Types)
        {
            EStringConversionResult status = TryConvertStringToType(passedArg, type, out object converted);
    ...
    {
        ILookup<StringName, CommandInputType> lookup = option.Types.ToLookup(t => t.Type);
        ...
        foreach (string passedOpt in passedOpts)
        {
            ...
            foreach (CommandInputType type in option.Types)
            {
                if (type.IsArray)
                {
                    ...
                    {
                        EStringConversionResult st = TryConvertStringToType(v, type, out object convertedLValue);
                ...
                EStringConversionResult status = TryConvertStringToType(value, type, out object converted);
    ...
    )
    {
        ...
        {
            bool status = TryConvertNumeric(value, type, out object r);
            result = r;
        ...
        if (t == "string")
        {
            if (type.Min != type.Max && !Numeric.IsWithinRange(value.Length, type.Min, type.Max)
               )
            {
    ...
    private static bool TryConvertNumeric(StringName value, CommandInputType type, out object? result)
    {
        ...
        if (type.Type == "int")
        {
            if (!Numeric.TryConvert(value, out int r))
            {
                return false;
            }
        ...
        if (type.Type == "float")
        {
            if (!Numeric.TryConvert(value, out float r))
            {
                return false;
            }
            ...
            if (type.Min != type.Max)
            {
                return Numeric.IsWithinRange<float, float>(
                    r, type.Min, type.Max
                );
            }
        }
