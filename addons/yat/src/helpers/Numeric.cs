...
public static class Numeric
{
    ...
    public static bool IsWithinRange<T, TE>(T value, TE min, TE max)
        where T : IComparable<T>, IComparable<TE> =>
        value.CompareTo(min) >= 0 && value.CompareTo(max) <= 0;
    ...
    /// <param name="output">
    ///     When this method returns, contains the converted value if the conversion succeeded, or the
    ///     fallback value if the conversion failed.
    /// </param>
    /// <param name="fallback">
    ///     The fallback value to use if the conversion fails. The default value of the type T is used if no
    ///     fallback value is specified.
    /// </param>
