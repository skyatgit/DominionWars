...
public static class Keybindings
{
    ...
    private static readonly Tuple<StringName, InputEvent>[] s_defaultActions =
    {
        new(TerminalToggle, new InputEventKey { PhysicalKeycode = Key.Quoteleft }),
        new(ContextMenu,
            new InputEventMouseButton { ButtonMask = MouseButtonMask.Right, ButtonIndex = MouseButton.Right }),
        ...
        new(TerminalInterrupt, new InputEventKey { PhysicalKeycode = Key.C, CtrlPressed = true }),
        new(TerminalAutocompletionNext, new InputEventKey { PhysicalKeycode = Key.Tab }),
        new(TerminalAutocompletionPrevious, new InputEventKey { PhysicalKeycode = Key.Tab, ShiftPressed = true }),
    ...
    {
        foreach (Tuple<StringName, InputEvent> action in s_defaultActions)
        {
