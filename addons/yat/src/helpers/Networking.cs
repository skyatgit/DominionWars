...
public static class Networking
{
    public static PingReply? Ping(string hostname, NetworkingOptions? options = null)
    {
        ...
        try
        {
            PingReply reply = ping.Send(hostname, options.Timeout, buffer,
                new PingOptions { Ttl = options.Ttl, DontFragment = options.DontFragment });
            return reply;
        }
