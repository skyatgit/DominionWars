...
public static class Parser
{
    ...
    public static (string, string[]) ParseMethod(string method)
    {
        ...
        string[] parts = (tokens.Length <= 1 ? tokens[^1] : tokens[0]).Split('(', 2,
    ...
    public static bool TryParseCommandInputType(string type, out CommandInputType parsed)
    {
        ...
        string[] tokens = type.Trim(')').Split('(', StringSplitOptions.RemoveEmptyEntries);
        ...
        {
            parsed = new CommandInputType(tokens[0], float.MinValue, float.MaxValue, isArray);
        }
        else
        {
            return false;
    ...
    private static bool TryParseTypeWithRange(string[] tokens, bool isArray, out CommandInputType parsed)
    {
        ...
        string[] minMax = tokens[1].Split(':', StringSplitOptions.RemoveEmptyEntries);
        ...
        if (minMax.Length == 1)
        {
            ...
            if (!isMaxPresent)
            {
                ...
                {
                    parsed = new CommandInputType(tokens[0], float.MinValue, max, isArray);
                }
                else
                {
                    return false;
            ...
            else // If only max value is not present, set it to float.MaxValue
            {
                ...
                {
                    parsed = new CommandInputType(tokens[0], min, float.MaxValue, isArray);
                }
                else
                {
                    return false;
        ...
        else if (minMax.Length > 0)
        {
            ...
            {
                parsed = new CommandInputType(tokens[0], min, max, isArray);
            }
            else
            {
                return false;
