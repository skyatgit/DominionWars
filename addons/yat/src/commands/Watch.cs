...
{
    private const uint SecondsMultiplier = 1000;

    public CommandResult Execute(CommandData data)
    {
        if (!RegisteredCommands.Registered.TryGetValue((string)data.Arguments["command"], out Type type))
        {
        ...
        float interval = (float)data.Arguments["interval"] * SecondsMultiplier;
