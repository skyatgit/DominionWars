...
public sealed class Ss : ICommand
{
    ...
    public CommandResult Execute(CommandData data)
    {
        ...
        string path = (string)data.Options["-path"] ?? OS.GetExecutablePath().GetBaseDir() + "/";
        string name = (string)data.Options["-name"] ?? $"{((DateTimeOffset)DateTime.UtcNow).ToUnixTimeMilliseconds()}";
