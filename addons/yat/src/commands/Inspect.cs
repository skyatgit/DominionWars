...
public sealed class Inspect : ICommand
{
    public CommandResult Execute(CommandData data)
    {
        ...
        if (useRayCast)
        {
            result = InspectRayCastedObject(World.RayCast(data.Yat.GetViewport()), all);
        }
        else
        {
            result = InspectNode(data.Terminal.SelectedNode.Current, all);
        }
    ...
    private static StringBuilder GetAllProperties(Node node)
    {
        ...
        Array<Dictionary> properties = node.GetPropertyList();
        ...
        foreach (Dictionary property in properties)
        {
            ...
            {
                sb.AppendLine(propertyData.AsString());
            }
        }
    ...
    private static StringBuilder InspectNode(Node node, bool all = false)
    {
        ...
        {
            sb.Append(InspectNode2D(node2D));
        }
        else if (node is Node3D node3D)
        {
            sb.Append(InspectNode3D(node3D));
        }
        else if (node is Control control)
        {
            sb.Append(InspectControl(control));
        }
