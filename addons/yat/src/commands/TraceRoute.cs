...
public sealed class TraceRoute : ICommand
{
    public CommandResult Execute(CommandData data)
    {
        ...
        NetworkingOptions options = new NetworkingOptions
        {
        ...
        IEnumerable<IPAddress> addresses = Networking.GetTraceRoute(hostname, options, data.CancellationToken);
        StringBuilder result = new StringBuilder();

        foreach (IPAddress address in addresses)
        {
            result.AppendLine(address?.ToString());
        }
